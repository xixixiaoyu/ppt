...
const strategies = [
  {
    ...
    items: [
      { name: '更小的分块 (Small-to-Big)', desc: '检索时使用小块文本以提高精准度，但给 LLM 时提供其所属的更大父文档块，补充上下文。' },
      { name: '元数据过滤 (Metadata Filtering)', desc: '为文档添加时间、作者、类别等元数据，检索时先过滤，缩小范围，提高准确率。' },
  ...
  },
  {
    ...
    items: [
      { name: '混合检索 (Hybrid Search)', desc: '结合关键词搜索 (BM25) 和向量搜索 (Vector Search)。关键词擅长精确匹配（如人名、型号），向量擅长语义理解。' },
      { name: '查询重写 (Query Rewriting)', desc: 'LLM 将用户的模糊问题改写为更适合检索的形式，或分解为多个子问题。' },
      { name: '假设性文档嵌入 (HyDE)', desc: '先让 LLM 生成一个假设性答案，再用该答案去检索相似的真实文档。' }
    ]
  },
  {
    id: 'post-retrieval',
    ...
    items: [
      { name: '重排序 (Reranking)', desc: '使用专门的 Rerank 模型（如 Cohere Rerank, BGE-Reranker）对召回的 Top-K 结果进行精细排序，将最相关的排在最前。' },
      { name: '上下文压缩', desc: '仅提取检索文档中与问题相关的关键句子，减少无关信息对 LLM 的干扰。' }
    ]
  }
]

const activeStrategy = ref('retrieval')
</script>

<template>
  <section class="container mx-auto max-w-6xl px-6 md:px-8 lg:px-12 py-12 lg:py-16 h-full flex flex-col">
    ...
    <div class="flex-1 flex flex-col lg:flex-row gap-6 min-h-0">
      ...
      <div class="lg:w-1/3 flex flex-col gap-3">
        ...
        >
          <div 
            ...
            :class="activeStrategy === strategy.id ? 'bg-accent/10 text-accent' : 'bg-slate-100 text-slate-500 group-hover:text-slate-700'"
      ...
      <!-- Content Area -->
      <div class="lg:w-2/3 bg-white/60 backdrop-blur-xl border border-white/50 rounded-2xl p-6 lg:p-8 shadow-xl flex flex-col justify-center">
        <transition name="fade" mode="out-in">
          <div :key="activeStrategy" class="space-y-6">
            ...
            <div class="mt-4 p-4 bg-yellow-50/80 border border-yellow-100 rounded-lg flex gap-3 text-sm text-yellow-800">
              <svg class="w-5 h-5 shrink-0 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              <span v-if="activeStrategy === 'data'">
                数据质量是 RAG 的上限。如果源数据混乱或切分不当，后续模型再强也无法弥补 (Garbage In, Garbage Out)。
              </span>
              <span v-else-if="activeStrategy === 'retrieval'">
                检索优化的核心是弥补“语义匹配”的不足，让机器更懂用户的真实意图。
              </span>
              <span v-else>
                重排序 (Rerank) 通常是提升 RAG 效果性价比最高的单一手段，虽然会增加少量延迟，但能显著提升 Top-1 准确率。
              </span>
            </div>
          </div>
        </transition>
