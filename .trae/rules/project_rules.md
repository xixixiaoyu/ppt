...
**最重要的一点**：项目在 `src/presentations/design-language-template` 目录下提供了一套预设的幻灯片模板。在创建新的幻灯片时，AI 应**始终优先复用**这些模板，而不是从零开始编写。

**工作流程**：
1.  **分析需求**：理解需要创建的幻灯片类型（如封面、目录、图表、问答等）。
2.  **匹配模板**：在 `design-language-template` 目录中寻找功能和布局最匹配的 `.vue` 文件。
3.  **复用与修改**：将选定的模板内容作为基础，仅修改文本、数据和少量样式以满足具体需求。
